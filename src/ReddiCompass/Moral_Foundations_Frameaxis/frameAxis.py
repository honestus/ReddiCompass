...
class FrameAxis:
    ...
    def doc_scores(self, docs, baseline_docs, tfidf=False):
        ...
        if baseline_docs:
            ...
            all_docs_tokens = [x if x in self.vocab else x.lower() if x.lower() in self.vocab else False for x in all_baseline_docs.split() if x in self.vocab]
            all_docs_tokens = [t for t in all_docs_tokens if t]
        for mf in self.axes.keys():
            #print(mf)
            mf_scores_bias = []
            mf_scores_intensity = []
            ...
            for idx in range(len(docs)):
                ...
                doc = docs[idx]
                doc_tokens = [x if x in self.vocab else x.lower() if x.lower() in self.vocab else False for x in doc.split() if x in self.vocab]
